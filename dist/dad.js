  ...
  return function () {
    ...
    date.format = function () {
      ...
      return str.replace(/[yY]+/, function (Y) {
        ...
        return DD.length > 1 ? prefix0(date.date) : date.date;
      }).replace(/h+/, function (h) {
        return h.length > 1 ? prefix0(date.hours) : date.hours;
      }).replace(/m+/, function (h) {
        return h.length > 1 ? prefix0(date.minutes) : date.minutes;
      }).replace(/s+/, function (h) {
        return h.length > 1 ? prefix0(date.seconds) : date.seconds;
      });
    };
